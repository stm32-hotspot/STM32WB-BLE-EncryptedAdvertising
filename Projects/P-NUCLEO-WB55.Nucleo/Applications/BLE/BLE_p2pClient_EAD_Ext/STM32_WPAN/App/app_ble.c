...
typedef struct
{
  ...
  uint8_t deviceServerFound;
  uint8_t KeyMaterialStoredinFlash;
  uint8_t DeviceAlreadyBonded;
...
#define FLASH_ADDR_KEY_MATERIAL (0x0807F000)
#define FLASH_ADDR_IV (0x0807F010)
/* USER CODE END PM */
...
uint64_t *stored_key_addr;
uint64_t *stored_init_vector_addr;
uint8_t key_byte[16];
uint8_t restored_key[16];
uint8_t restored_iv[8];
uint32_t flash_fsm_state;
...
void APP_BLE_Init(void)
{
  ...
  /* check if Key Material characteristic has been stored */
      
  stored_key_addr = (uint64_t *)FLASH_ADDR_KEY_MATERIAL;
  stored_init_vector_addr = (uint64_t *)FLASH_ADDR_IV;
      
  if((*stored_key_addr) != (uint64_t)0xFFFFFFFFFFFFFFFF)
  {
    /* restore key and iv to be used in aci_hal_ead_encrypt_decrypt */
    memcpy(restored_key, &(*stored_key_addr), 16);
    memcpy(restored_iv, &(*stored_init_vector_addr),8);
    BleApplicationContext.DeviceAlreadyBonded = 1;
  }
  else
  {
    BleApplicationContext.DeviceAlreadyBonded = 0;
    APP_DBG_MSG("  No Stored Key Material Characteristic \n\r");
  }
      ...
      switch (blecore_evt->ecode)
      {
        ...
        case (ACI_GAP_NUMERIC_COMPARISON_VALUE_VSEVT_CODE):
          ...
          APP_DBG_MSG("     - Hex_value = %lx\n",
                      ((aci_gap_numeric_comparison_value_event_rp0 *)(blecore_evt->data))->Numeric_Value);
          result = aci_gap_numeric_comparison_value_confirm_yesno(BleApplicationContext.BleApplicationContext_legacy.connectionHandle, YES); /* CONFIRM_YES = 1 */
        ...
        case (ACI_GAP_PAIRING_COMPLETE_VSEVT_CODE):
        {
          ...
          if (pairing_complete->Status == 0)
          {
            ...
            result = aci_gatt_exchange_config(BleApplicationContext.BleApplicationContext_legacy.connectionHandle);
            if (result != BLE_STATUS_SUCCESS)
            {
      ...
      case HCI_LE_ENHANCED_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          result = aci_gap_is_device_bonded(p_enhanced_connection_complete_event->Peer_Address_Type, p_enhanced_connection_complete_event->Peer_Address);
          if (result == BLE_STATUS_SUCCESS)
          {
        ...
        { 
          ...
          while ( k < ext_adv_evt->Data_Length )
          {
            ...
            /* get encrypted advertising data */
            if ( adtype == AD_TYPE_ENCRYPTED_ADV_DATA)
            {
              uint16_t j;
              APP_DBG_MSG("  \r\n");
              APP_DBG_MSG("  === encrypted data received \n");
              APP_DBG_MSG("  \r\n");
              BleApplicationContext.EAD_found = 1;
              BleApplicationContext.EAD_length = adlength;

              for (j=0;j<BleApplicationContext.EAD_length - 1;j++)
              {
                BleApplicationContext.EAD_data[j] = ext_adv_evt->Data[j+k+2];
              }
              /* call decryption function */

              if (BleApplicationContext.DeviceAlreadyBonded)
              {
                APP_DBG_MSG("  === Use of encryption key stored in Flash \n");
                result = aci_hal_ead_encrypt_decrypt(0x01, restored_key, restored_iv, adlength - 1, BleApplicationContext.EAD_data, &Out_Data_Length, Out_Data);
              }
              else
              {
                APP_DBG_MSG("  === Use of encryption key stored in RAM \n");
                result = aci_hal_ead_encrypt_decrypt(0x01, encryption_key, initialization_vector, adlength - 1, BleApplicationContext.EAD_data, &Out_Data_Length, Out_Data);
              }
              if (result != BLE_STATUS_SUCCESS)
              {
                APP_DBG_MSG("  === Fail   : aci_hal_ead_encrypt_decrypt, result: 0x%02X\n", result);
                APP_DBG_MSG("  === expected at first connection if devices not bonded \r\n");
                APP_DBG_MSG("  \r\n");
              }
              else
              {
            ...
            if ( adtype == AD_TYPE_MANUFACTURER_SPECIFIC_DATA )
            {
              ...
              {
                if (ext_adv_evt->Data[k + 3] == CFG_DEV_ID_P2P_EADS)
                {
...
void APP_BLE_Key_Button2_Action(void)
{
  ...
  else
  {
    ...
    {
      APP_DBG_MSG("aci_gap_clear_security_db fail, status =0x%02X\n\n", result);
    } 
    *restored_key = (uint64_t)(0x00);
    *restored_iv = (uint64_t)(0x00);
    flash_fsm_state = APP_BLE_Erase_Flash;
...
static void BLE_SVC_GAP_Security_Req( void )
{
  ...
  status = aci_gap_send_pairing_req(BleApplicationContext.BleApplicationContext_legacy.connectionHandle, 0x01);
...
static void FlashOperationProc(void)
{
  uint32_t NbrOfSectorNotErased, NbrOfDataNotWritten;//, first_secure_sector_idx;

  first_secure_sector_idx = 127;
  
  if(flash_fsm_state == APP_BLE_EraseBeforeWrite_Flash)
  {
     NbrOfSectorNotErased = FD_EraseSectors(first_secure_sector_idx , 1);
     if(NbrOfSectorNotErased != 0)
     {
       UTIL_SEQ_SetTask( 1<<CFG_TASK_FLASH_OPERATION_REQ_ID, CFG_SCH_PRIO_0);
     }
     else
  ...
  else if(flash_fsm_state == APP_BLE_Write_Flash)
  {
    NbrOfDataNotWritten = FD_WriteData(FLASH_ADDR_KEY_MATERIAL,
                                        FlashDataToWriteTab,
                                        3);
     
     if(NbrOfDataNotWritten != 0)
     {
       UTIL_SEQ_SetTask( 1<<CFG_TASK_FLASH_OPERATION_REQ_ID, CFG_SCH_PRIO_0);
     }
     else
     {
       flash_fsm_state = APP_BLE_NoAction;
     }
  }
  else if(flash_fsm_state == APP_BLE_Erase_Flash)
  {
    NbrOfSectorNotErased = FD_EraseSectors(first_secure_sector_idx , 1);
    if(NbrOfSectorNotErased != 0)
    {
