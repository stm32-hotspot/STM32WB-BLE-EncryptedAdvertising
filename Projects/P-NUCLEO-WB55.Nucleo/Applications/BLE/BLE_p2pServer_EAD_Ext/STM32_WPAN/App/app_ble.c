...
uint8_t a_ManufData[14] = {sizeof(a_ManufData)-1,
                           ...
                           0x01,                               /*SKD version */
                           CFG_DEV_ID_P2P_EADS,             /* STM32WB - P2P Server 1*/
...
//const uint8_t ead_AdvData[7] = {0x06, AD_TYPE_ENCRYPTED_ADV_DATA, 0x62,0x6F,0x6E,0x6A,0x6F};
uint8_t ead_AdvData[171];
uint8_t ead_data_encrypted[228];
const uint8_t *ead_data_decrypted;
extern uint8_t ead_data_encryp[180];
uint16_t ead_data_encrypted_length;
uint8_t a_AdvData[22] =
{
  6, AD_TYPE_COMPLETE_LOCAL_NAME, 'E', 'A', 'D', 'S',  /* Complete name */
  16, AD_TYPE_MANUFACTURER_SPECIFIC_DATA, 0x30, 0x00, 0x00 /*  */, 0x00 /*  */, 0x00 /*  */, 0x00 /*  */, 0x00 /*  */, 0x00 /*  */, 0x00 /*  */, 0x00 /*  */, 0x00 /*  */, 0x00 /*  */, 0x00 /*  */, 0x00 /*  */,
};
/* USER CODE END PFP */
...
void APP_BLE_Init(void)
{
  ...
  UTIL_SEQ_RegTask(1<<CFG_TASK_ADV_RESTART_ID, UTIL_SEQ_RFU, restart_adv);
...
SVCCTL_UserEvtFlowStatus_t SVCCTL_App_Notification(void *p_Pckt)
{
  ...
  switch (p_event_pckt->evt)
  {
    case HCI_DISCONNECTION_COMPLETE_EVT_CODE:
    {
      ...
      UTIL_SEQ_SetTask( 1<<CFG_TASK_ADV_RESTART_ID, CFG_SCH_PRIO_0);
...
static void Ble_Hci_Gap_Gatt_Init(void)
{
 ...
 /* Add a new characterisitc "Encrypted Data Key Material" */
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_hal_write_config_data command - CONFIG_DATA_GAP_ADD_REC_NBR_OFFSET, result: 0x%02X\n", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_hal_write_config_data command - CONFIG_DATA_GAP_ADD_REC_NBR_OFFSET\n");
  }
...
/* USER CODE END Role_Mngt */
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : encryption_key+iv, error code: 0x%2X\n", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : encryption_key+iv\n");
  }
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_update_char_value - EncrDataKayMaterial, result: 0x%02X\n", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_update_char_value - EncrDataKayMaterial\n");
  }
  for (i=0;i<171;i++)
  {
    ead_AdvData[i] = i;
